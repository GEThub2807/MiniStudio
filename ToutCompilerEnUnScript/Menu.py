...
def credit():
    ...
    while True:
        ...
        RECT_CREDIT_BS_TEXT = CREDIT_BS_TEXT.get_rect(
            center=(SCREEN_WIDTH / 2, vertical_position + 2 * vertical_spacing))
        RECT_BS = BS.get_rect(center=(SCREEN_WIDTH / 2, vertical_position + 3 * vertical_spacing))
        RECT_CREDIT_ART_TEXT = CREDIT_ART_TEXT.get_rect(
            center=(SCREEN_WIDTH / 2, vertical_position + 4 * vertical_spacing))
...
def options():
    ...
    while True:
        ...
        volume_slider_x, volume_slider_y, (volume / volume_max) * volume_slider_width, volume_slider_height))
        Adjust_Pose = 0.64
        OPTIONS_TEXT = get_font(45).render("This is the OPTIONS screen.", True, "Black")
        VOLUME = get_font(75).render("VOLUME", True, "Black")  # Texte pour le volume
        RECT_VOLUME = VOLUME.get_rect(
            center=(SCREEN_WIDTH / 2, SCREEN_HEIGHT * Adjust_Pose))  # Rectangle pour centrer le texte
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if volume_slider_x <= OPTIONS_MOUSE_POS[0] <= volume_slider_x + volume_slider_width and \
                    ...
                    volume = max(0, min((OPTIONS_MOUSE_POS[0] - volume_slider_x) / volume_slider_width * volume_max,
                                        volume_max))
